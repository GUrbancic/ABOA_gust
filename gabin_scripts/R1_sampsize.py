...
# Read in raw data and perform comparitive error analysis for sampling error, 
# spike detection, unrealistic values. 
...
for fname in fnames:
    ...
    for i in u1hr:
        ...
        for m in u10min_h :
            ...
            msk4 = np.floor(tim[:]/1.0e+3) == m/1.0e+3
            ...
            msk2 = np.floor(time[:]/1.0e+3) == m/1.0e+3
            sec = np.unique(time[msk2])
            
            for ti in sec:
                msk3 = time[msk2] == ti
                if len(msk3[msk3]) == 20 and len(sec) == 600 and datgood == True:
                    datgood = True
                else: 
                    datgood = False
                    break
    ...
    np.savetxt(datdir+ 'quality/' + 'fil_cnt_'+str(day)+'.txt',np.c_[ \
